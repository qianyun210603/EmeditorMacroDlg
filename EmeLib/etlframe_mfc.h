...
#define EE_DLL_H(Type)  extern "C" Type __stdcall
#define _ETL_IMPLEMENT                                                                                      \
    CETLFrameX* _ETLCreateFrame() { CETLFrameX* pFrame = new ETL_FRAME_CLASS_NAME; return pFrame; }         \
    void _ETLDeleteFrame( CETLFrameX* pFrame ) { delete static_cast<ETL_FRAME_CLASS_NAME*>(pFrame); }       \
    class CETLData _ETLData;                                                                                \
    EE_DLL_H(void) OnCommand( HWND hwndView ) { OnCommand_<int>(hwndView); }                                \
    EE_DLL_H(BOOL) QueryStatus( HWND hwndView, LPBOOL pbChecked ) { return QueryStatus_<int>(hwndView, pbChecked); } \
    EE_DLL_H(UINT) GetMenuTextID() { return GetMenuTextID_<int>(); }                                        \
    EE_DLL_H(UINT) GetStatusMessageID() { return GetStatusMessageID_<int>(); }                              \
    EE_DLL_H(UINT) GetBitmapID() { return GetBitmapID_<int>(); }                                            \
    EE_DLL_H(void) OnEvents( HWND hwndView, UINT nEvent, LPARAM lParam ) { OnEvents_<int>(hwndView, nEvent, lParam); } \
    EE_DLL_H(LRESULT) PlugInProc( HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam )                      \
        { return PlugInProc_<int>(hwnd, nMsg, wParam, lParam); }
...
//	#ifdef __USE_MFC__
//	extern void OnDllProcess_AttachDetach(bool bAttach);
//	#endif

#ifndef EE_EXTERN_ONLY
inline HINSTANCE EEGetLocaleInstanceHandle()
{
	CETLLock lock;
	{
...
inline void GetModuleFilePath( LPCTSTR szFile, LPTSTR szPath )
{
	szPath[0] = '\0';
...
inline HINSTANCE GetInstancePath( LPCTSTR szPath )
{
	_ASSERT( szPath[0] );
...
inline CETLFrameX* GetFrameFromFrame( HWND hwndFrame )
{
	_ASSERTE( hwndFrame );
...
inline CETLFrameX* GetFrame( HWND hwnd )
{
...
inline CETLFrameX* GetFrameFromDlg( HWND hwnd )
{
	return GetFrame( hwnd );
}

inline CETLFrameX* GetFrameFromView( HWND hwndView )
{
...
//user must call the func in InitInstance() and ExitInstance()
inline void OnDllProcess_AttachDetach(bool bAttach)
{
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
    if (bAttach) {
		_ASSERTE( _ETLData.m_hInstance == NULL );
		_ETLData.m_hInstance = AfxGetApp()->m_hInstance;  //AfxGetInstanceHandle()
	}
	else {
		_ASSERTE( _ETLData.m_pETLFrameMap == NULL );
	}
...
template<typename TT> void OnCommand_( HWND hwndView )
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> BOOL QueryStatus_( HWND hwndView, LPBOOL pbChecked )
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> UINT GetMenuTextID_()
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> UINT GetStatusMessageID_()
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> UINT GetBitmapID_()
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> void OnEvents_( HWND hwndView, UINT nEvent, LPARAM lParam )
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
...
template<typename TT> LRESULT PlugInProc_( HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam )
{
#ifdef __USE_MFC__
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
#endif
